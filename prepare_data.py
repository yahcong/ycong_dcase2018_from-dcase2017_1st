...
#changed by ycong,cause,mete_data changed
def pack_features_to_hdf5(fe_dir, csv_path, out_path):
    ...
    structure:
       na_list: list of names,such as :00pK0GMmE9s_70.000_80.000.wav,remove 'y'
    ...
    if csv_path != "":    # Pack from csv file (training & testing from dev. data)
        ...
        #print(lis)
        #remove first line in csv: "filename onset offset event_label"
        lis = lis[1:]

        #ycong to merge test items when same name
        prev_na = ''
        prev_ids = []
        prev_li = []
        merge_lis = []
        for li in lis:
            ...
            if na != prev_na:
                ...
                if merge_lis[-1][-1] != li[-1]:
                    merge_lis[-1][-1]=merge_lis[-1][-1]+','+li[-1]
        print('len merge_lis:'+str(len(merge_lis)))
        lis=merge_lis
        ...
        for li in lis:
            ...
            #mete data中没有给ibs,只给了lbs，这里将lbs就当作ids
	    #lbs并没有存储到hd5文件中，并没有起作用,而是ibs起作用，是否需要将lbs转为ibs
            #[na, bgn, fin, ids] = li
            
            if cnt % 100 == 0: print(cnt)
            na = os.path.splitext(na)[0]
            fe_na = na + ".p" #correspond to the features name
            ...
            if not os.path.isfile(fe_path):
                ...
                na_all.append(na+".wav")#do not remove 'Y',correspond to the wav name
    ...
    #na_all的类型不对，create_dataset时出错，这里ycong添加一行代码，尝试转换为bool格式，但是并不确定该转换为什么格式
    na_all =  np.array(na_all, dtype=np.string_)
    with h5py.File(out_path, 'w') as hf:
        #print('na_list'+str(na_all))
        #print('y_all:'+str(y_all))
