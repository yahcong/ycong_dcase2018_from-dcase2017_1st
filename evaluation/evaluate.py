...
from evaluation.evaluation_modified_ankitshah009.Models import FileFormat
import evaluation.evaluation_qk.evaluation as eva
from evaluation.io_task4 import create_folder, get_my_open
from evaluation.io_task4 import at_read_prob_mat_csv, at_read_submission_csv, at_read_weak_gt_csv
from evaluation.io_task4 import sed_read_prob_mat_list_to_csv, sed_read_submission_csv_to_prob_mat_list, sed_read_strong_gt_csv
...
class AudioTaggingEvaluate(object):
    ...
    def get_stats_from_prob_mat_csv(self, pd_prob_mat_csv, thres_ary):
        ...
        #print('gt_na_list:'+str(gt_na_list))
        #print('pd_na_list:'+str(pd_na_list))
        pd_prob_mat = self._reorder_mat(pd_prob_mat, pd_na_list, gt_na_list)
        print('gt_mat:'+str(gt_mat))
        print('pd_prob_mat:'+str(pd_prob_mat))
...
class SoundEventDetectionEvaluate(object):
    ...
    def get_stats_from_prob_mat_list(self, pd_prob_mat_list, gt_mat_list, thres_ary):
        ...
        for n in range(N):
            ...
            print('pd_prob_mat:'+str(pd_prob_mat.shape))
            pd_digit_mat = self._get_digit_mat_from_thres_ary(pd_prob_mat, thres_ary)
